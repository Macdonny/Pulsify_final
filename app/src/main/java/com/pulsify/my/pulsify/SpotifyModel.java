package com.pulsify.my.pulsify;

/**
 * Created by Krpto_Mac on 10/23/2016.
 */

public class SpotifyModel {
    public String excited = "spotify:track:6NPVjNh8Jhru9xOmyQigds";
    public String angry = "spotify:track:03UrZgTINDqvnUMbbIMhql";
    public String relaxed = "spotify:track:7L7KnCltNM8yXWs3ozuUY1";
    public String crazy = "spotify:track:11k3FBcoJikSIGncMD656T";
    public String sleepy = "spotify:track:2Lyw5hRHymLpzWBrG056Lz";
    public String thoughtful = "spotify:track:53KU8BrYZuxZOb3jqDm4ix";
    public String jazzy = "spotify:track:1VKsbTJ78G5bnfyoPz46LA";
    public String calm = "spotify:track:28VwmnD29YNEMh6OwI2uLH";
    public String dance = "spotify:track:1lItf5ZXJc1by9SbPeljFd";
    public String mario = "spotify:track:1oLFqOLGARvpOoKANj1wDa";
    public String peter = "spotify:track:0x32ZUWjGSXOZXU7gMduuP";
    public String burns = "spotify:track:6YCBTYhsZZgze0AfNcbdZS";

    public class Song {
        private final String track;

        public Song(String track){
            this.track = track;
        }
    }
}
